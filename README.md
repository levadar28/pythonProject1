...
Вывод:  изучила основы объектно-ориентированного программирования в Python, научилась описывать модели данных с помощью @dataclass, реализовывать методы и валидацию, сериализовывать/десериализовывать объекты.
...
###  Примеры работы
<img width="1125" height="674" alt="group laba09 Kolesnichenko Daria" src="https://github.com/user-attachments/assets/919aed29-6698-49be-94a7-13272cecd5fa" />
<img width="600" height="949" alt="group (2) laba09 Kolesnichenko Daria" src="https://github.com/user-attachments/assets/bdf518d9-9948-472b-a334-8d3b480cc9ba" />
<img width="1617" height="1058" alt="group (3) laba09 Kolesnichenko Daria" src="https://github.com/user-attachments/assets/d41cc975-207c-40eb-aeaa-91523e5a1eff" />
...
1) Основная статистика (по заданию):
...
### Примеры работы
<img width="1734" height="1083" alt="stats laba09 Kolesnichenko Daria" src="https://github.com/user-attachments/assets/16584bcf-b48b-4c55-8890-06e84f19c2e0" />
<img width="1553" height="1058" alt="stats (2) laba09 Kolesnichenko Daria" src="https://github.com/user-attachments/assets/a0c27782-7e3e-432f-8105-3ea7ad2c6f5b" />

Вывод: реализовала простейшее хранилище данных студентов на основе CSV-файла, отработала CRUD-операции (Create / Read / Update / Delete) и научилась работать с ними через отдельный класс `Group`.
...
* Использует список self._data для хранения элементов.

* Вершина стека — последний элемент списка (self._data[-1]).

* Метод pop() вызывает IndexError с понятным сообщением при пустом стеке.

* Метод peek() возвращает None для пустого стека.

* Реализованы все требуемые методы и опциональный __len__.

2. Queue:

* Использует collections.deque для эффективных операций с обоих концов.

* Метод dequeue() вызывает IndexError с понятным сообщением при пустой очереди.

* Метод peek() возвращает None для пустой очереди.

* Реализованы все требуемые методы и опциональный __len__.

3. Дополнительно:

* Добавлены аннотации типов для лучшей читаемости.

* Реализованы __str__ методы для удобного отображения объектов.

* Исключения содержат понятные сообщения на русском языке.

* Код соответствует PEP 8 и содержит документационные строки.

### Пример выполнения работы
<img width="1920" height="1200" alt="structures laba10 Kolesnichenko Daria" src="https://github.com/user-attachments/assets/efadf59b-8a08-4947-87b6-b3a9eb7e76bc" />

### Задание B
```python
```
### Основные особенности реализации
Атрибут self.tail: для ускорения операции append() до O(1).
Управление размером: в каждом методе добавления/удаления корректно обновляется self._size.
Проверка индексов: в методах insert() и remove_at() проверяются границы индексов.
Полная обработка крайних случаев: Пустой список, удаление головы/хвоста.
Реализованы оба метода удаления: remove() (по значению) и remove_at() (по индексу).
Улучшенные аннотации типов: для лучшей читаемости кода.
Оптимизированная вставка: использует prepend() и append() для вставки в начало/конец.

Сложность операций:

* append(), prepend(): O(1)

* insert(): O(n) в худшем случае (вставка в середину)

* remove(): O(n) (поиск элемента)

* remove_at(): O(n) (доступ по индексу)

* __iter__(): O(n)

* __len__(): O(1)

* remove() ничего не делает, если значение не найдено;

* insert() поддерживает вставку в конец списка (idx = len(list));

Оба метода удаления корректно обновляют tail при удалении последнего элемента.

Представление списка выглядит как SinglyLinkedList([1, 2, 3]).

### Пример выполнения работы
<img width="1695" height="1011" alt="linked list laba10 Kolesnichenko Daria" src="https://github.com/user-attachments/assets/d2fc9aca-1a99-4b0d-b5b4-e0578af01173" />

Вывод: реализовала базовые структуры данных (стек, очередь, связный список), сравнила их производительность и научилась думать в терминах асимптотики (O(1), O(n)). 
